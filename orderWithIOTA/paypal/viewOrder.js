...
function start(){
  ...
  $.ajax({
    ...
    success: function(reData){     //成功的時候跑的函式
	 ...
	 $("#navPoint").attr("href", "http://140.127.74.191:3000/SQL/" +account);
    },
...
$(document).ready(function(){
  ...
  $.ajax({
    url:"https://140.127.74.191/wtlab108/orderWithIOTA/php/getOrder.php",  //呼叫的位址
    type:"GET",      //請求方式
    data:{},//傳送到server的資料
    dataType:'json', //預期server回傳的形式
    async:false,     //非同步
    success: function(reData){     //成功的時候跑的函式
      ...
      for( var i=0 ; i<reData.length ; i++){
        ...
        //兩個按鈕
        orderTbody += "<td><a class='btn btn-primary' href='https://140.127.74.191/wtlab108/orderWithIOTA/viewOrderDetail.php?orderID="+reData[i][0]+"'>查看訂單明細</a>&nbsp;&nbsp;";

        if(status=="交易中"){
        orderTbody += "<a class='btn btn-primary' href='https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=https://140.127.74.191/wtlab108/orderWithIOTA/php/QR.php?orderID="+reData[i][0]+"'>顯示QRCODE</a>";
        }
        orderTbody += "</td>";
        orderTbody+= "</tr>";
      }
...
function logout(){
	...
	document.cookie = "token=null;expires="+now.toGMTString()+";path=/wtlab108;domain=140.127.74.191";
