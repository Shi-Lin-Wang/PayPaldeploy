...
$(document).ready(function(){
  ...
  if(getCookie("token") != "" &&getCookie("token") != null ){
      if(!getCookie("authOver")){
          ...
          alert("等候逾時，請重新登入");
          window.location = "https://140.127.74.191/wtlab108/index.html";
      }
  }
  ...
  var url=window.location.toString(); //取得當前網址
    ...
    console.log(storeID);
  $.ajax({
    url:"php/getCartDetail.php",  //呼叫的位址
    type:"GET",      //請求方式
...
function sendOrder2(){
  if($('#cash').prop("checked")){
      //alert("cash");
      window.location="php/CartToOrder.php?storeID="+storeID;
      ...
      window.location="paypal/paypal.php?storeID="+storeID;
...
function sendOrder(){
  ...
  $.ajax({
    url:"php/getCartAmount.php",  //呼叫的位址
    type:"GET",      //請求方式
   ...
   if(cartAmount>0){
     ...
     $.ajax({
       ...
       data:{"customer":account_haruUse,
             "orderID":orderID,
             "orderCheck":"https://140.127.74.191/wtlab108/orderWithIOTA/orderCheck.php?orderID="+orderID},//傳送到server的資料
       dataType:'text', //預期server回傳的形式
       async:false,     //非同步
       success:{},
       error: function(jqXHR, textStatus, errorThrown){    //失敗的時候跑的函式
